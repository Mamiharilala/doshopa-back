				...
				if (pageField.get(key).getType().trim().toUpperCase().compareTo("POPUP") == 0) {
					...
					if(this.getIntervalleField().containsKey(key)) {
						this.setSearchForm(Utility.stringWithoutNull(this.getSearchForm())+ "<input name='isint" 
							 +pageField.get(key).getField().getName()+"' type='hidden' class='form-control' value='1' />");
						...
						this.setSearchForm(Utility.stringWithoutNull(this.getSearchForm())+ "<input name='isint" 
								 +pageField.get(key).getField().getName()+"' type='hidden' class='form-control' value='0' />");
						this.setSearchForm(Utility.stringWithoutNull(this.getSearchForm())
								+ "<div class='col-md-4 col-sm-12'><div class='form-group'><label>"
	...
	public void loadResult(HttpServletRequest request) throws Exception {
		...
		for (int i=0;i<fields.length;i++) {
			key = fields[i].getName();
 			if(request.getParameter("isint"+key)!=null&&request.getParameter("isint"+key).trim().compareTo("1")==0&&request.getParameter(key+"1")!=null&&request.getParameter(key+"2")!=null&&request.getParameter(key+"1")!=""&&request.getParameter(key+"2")!="") {
 			...
 			if( fields[i].getName().toUpperCase().compareTo("SCHEMA") != 0 && fields[i].getName().compareTo("TABLENAME") != 0

		    ...
		    if(pageField.get(key).isVisible()) {
		    	...
		    	if(pageField.get(key).getType().trim().toUpperCase().compareTo("TEXTAREA")==0) {
		    		this.setLineForm(Utility.stringWithoutNull(this.getLineForm())+
							"<div class='form-group row'><label class='col-sm-12 col-md-2 col-form-label'>"+pageField.get(key).getNameDisplay()+"</label><div class='col-sm-12 col-md-10'> <textarea name="+pageField.get(key).getField().getName()+" class='form-control'>"+val+"</textarea></div></div>"								
								);
		    	}else {
		    		this.setLineForm(Utility.stringWithoutNull(this.getLineForm())+
